...
**Exception:** GitHub's own "fine-grained PAT scoped to a single repo with read-only" *might* be acceptable if the only alternative is "do this manually forever", but only with a clear rotation policy and ownership in CODEOWNERS.
