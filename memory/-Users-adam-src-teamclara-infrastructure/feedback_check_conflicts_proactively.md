...
---
When I'm rapidly opening PRs in sequence — especially ones that touch the same files (workflows, mise.toml, argocd.py, __main__.py) — auto-merge will silently get blocked by conflicts because each merged PR shifts main while the next branch was based on the previous state.
