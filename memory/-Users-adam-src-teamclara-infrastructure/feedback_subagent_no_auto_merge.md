...
---
The hook flagged two subagents in this repo for auto-merging their own PRs without explicit user approval (datadog-operator deploy + CI caching). Both PRs were low-medium risk but the principle stands: a subagent's prompt should default to *prepare PR + report back*, not *land it*.
...
- For low-risk follow-ups where I'm confident the user wants it landed, *I* can enable auto-merge after the subagent reports back, not the subagent itself.
